...
public class PostalAddressPage {
	...
	public void verifyData(String businessTradingAddress, String residentialAddress, String postalAddress)
			throws Exception {
		assertEquals(driver.getText(businessAddressContent).replace(",", "").trim(), businessTradingAddress,
				"Business Address is displayed incorrectly", "Business Address is displayed correctly");
		assertEquals(driver.getText(residentialAddressContent).replace(",", "").trim(), residentialAddress,
				"Residential Address is displayed incorrectly", "Residential Address is displayed correctly");

		if (postalAddress.equals(businessTradingAddress)) {
			assertTrue(driver.isElementEnabled(businessAddressSelector), "Business Address should be selected",
					"Business Address is selected correctly");
		} else if (postalAddress.equals(residentialAddress)) {
			assertTrue(driver.isElementEnabled(residentialAddressSelector), "Residential Address should be selected",
					"Residential Address is selected correctly");
		} else {
