...
public class CreditCardAPI extends FinstroAPI{
	...
	public void saveCard(JSONObject card) throws Exception {
			...
			APIRequest response = new APIRequest().baseUrl(Constant.API_HOST).path(saveCardEnpoint).addHeader("Content-Type", "application/json")
					.oauth2(accessToken).body(card.toString()).post();
			
			Assert.assertTrue(response.getStatusCode() == 200, "Save card unsuccessfully");
	}
